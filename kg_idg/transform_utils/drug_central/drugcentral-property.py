...
if type_str == "JOURNAL ARTICLE":
    ice = Article(id=uri, type=type_str, published_in="")  # filled later from reference table
elif type_str == "BOOK":
    ice = Book(id=uri, type=type_str)
elif type_str in ["CLINICAL TRIAL", "DRUG LABEL"]:
    ice = Publication(id=uri, type=type_str)
elif type_str == "ONLINE RESOURCE":
    ice = InformationResource(id=uri, type=type_str)
elif type_str == "PATENT":
    patent_id = "GOOGLE_PATENT:" + str(row["document_id"]).replace(" ", "")
    ice = InformationContentEntity(id=uri, type=type_str)
else:
    ice = InformationContentEntity(id=uri, type=type_str)
