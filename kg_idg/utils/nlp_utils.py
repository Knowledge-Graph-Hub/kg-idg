...
def process_oger_output(path: str, input_file_name: str) -> pd.DataFrame:
    ...
    interested_df = sub_df.loc[
        (df["TokenizedTerm"] == df["PreferredTerm"].str.replace(r"\(.*\)", ""))
    ]
    interested_df = interested_df.drop(columns=["PreferredTerm"]).drop_duplicates()
    interested_df.to_csv(
        os.path.join(path, "output", input_file_name + "Filtered.tsv"), sep="\t", index=False
    )
