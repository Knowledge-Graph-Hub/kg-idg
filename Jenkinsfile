            ...
            steps {
                dir('./run_embedding') {
                    script{
                        ...
                        def EXIT_CODE=sh script:"gcloud compute ssh $GCLOUD_VM --zone $GCLOUD_ZONE --ssh-flag=\"-tt\" --command=\"sudo runuser -l jtr4v -c \'export PATH=~/anaconda3/bin:$PATH conda activate && cd NEAT && LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 neat run --config ${NEAT_YAML_FULL_REMOTE_PATH} |& tee -a /home/jtr4v/neat_output.txt\'\"", returnStatus:true
