            ...
            steps {
                dir('./run_embedding') {
                    script{
                        ...
                        def EXIT_CODE=sh script:'gcloud compute ssh $GCLOUD_VM --zone $GCLOUD_ZONE --ssh-flag="-tt" --command="sudo runuser -l jtr4v -c \'source ~/.bashrc && conda activate && cd NEAT && LC_ALL=en_US.UTF-8 LANG=en_US.UTF-8 neat run --config kg-idg.yaml &> /home/jtr4v/neat_output.txt\'"', returnStatus:true
